...
(function($) {
  ...
  $.initTripleLayout = function() {
    ...
    var container = document.getElementById('container'),
      ...
      createNavigation, gotoLeftPage, gotoRightPage, cleanClasses, navigate;
    ...
    if (!window.classicLayout) {
      ...
      navigate = function(dir) {
        ...
        var invDir, l, r, nextE, nextSection, nextLink, onTransitionEndFn;

        invDir = dir === 'right' ? 'left' : 'right';
        classie.add(container, 'vs-move-' + invDir);

        l = current === 0 ? sectionsCount - 1 : current - 1;
      ...
      $.address.change(function() {
        ...
        var dir, first_page_url, current, url_index, current_index, total, bi_dir_way, circular_way, shortest_way, url,
        navigateInterval = function(i, dir) {
          setTimeout(function() {
            navigate(dir);
          }, i * 400);
        };
        ...
        if (url === '') {
          first_page_url = $('.vs-nav li:first a').attr('href');
          url = first_page_url.slice(2, first_page_url.length);
        }

        // detect current page
        current = $('.vs-nav .vs-nav-current a').attr('href');
        current = current.slice(2, current.length);

        //get indexes
        url_index = $('.vs-nav a[href="#/' + url + '"]').parent().index();
        current_index = $('.vs-nav a[href="#/' + current + '"]').parent().index();
        ...
        // find shortest way
        total = $('.vs-nav li').length;

        // calculate bi-directional way
        bi_dir_way = url_index - current_index;
        ...
        // find shortest way
        shortest_way = Math.abs(bi_dir_way) < Math.abs(circular_way) ? bi_dir_way : circular_way;
        ...
        for (i = 0; i < shortest_way; i++) {
          navigateInterval(i, dir);
        }
        if (!normal_navigate) {
          setTimeout(function() {
            $html.removeClass('fast-navigate');
          }, shortest_way * 400);
        } else {
